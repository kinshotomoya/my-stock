...
class QuandleApiRepositoryImpl @Inject()(actorSystem: ActorSystem) extends QuandleApiRepository {
  ...
  override def getStocks(codes: List[StockCode], frequency: Frequency = Frequency.ANNUAL)(implicit requestBuilder: Builder[DataSetRequest]): Future[List[QuandlResult]] = {
    ...
    Future.firstCompletedOf(Seq(delayed, Nested(future).map(_.convertToDomainObject).value))
      .recoverWith{
      case _ => Future.successful(Nil)
    }
  }

  private def delayed(implicit ec: ExecutionContext): Future[Nothing] = akka.pattern.after(FiniteDuration(5000L, TimeUnit.MILLISECONDS), actorSystem.scheduler)(Future.failed(TimeOutError("timeout error")))
